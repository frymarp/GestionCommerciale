...
namespace GestionCommerciale.Api.Endpoints
{
    ...
    public static class OrdersEndpoints 
    {
        ...
        public static void MapOrdersEndpoints(this WebApplication app)
        {
            ...
            group.MapGet("/", async (IOrderRepository repo, ICurrentOrganizationProvider organization) =>
                Results.Ok(await repo.ListAsync(organization.GetOrganizationId())))
                .WithSummary("Liste toute les commandes.")
                .WithDescription("Retourne la liste complète des commandes enregistrées d'une organisation, sans filtre ni pagination.");
            ...
            // d'atteindre ce code.
            group.MapGet("/{id:guid}", async ([Description("Identifiant de la commande à récupérer.")] Guid id, IOrderRepository repo, ICurrentOrganizationProvider organization) =>
                await repo.GetAsync(id,organization.GetOrganizationId()) is { } order ? 
                Results.Ok(order)
                : Results.NotFound())
                .WithSummary("Récupère une commande par son Id.")
                .WithDescription("Retourne 404 si aucune commande ne correspond à l'Id fourni et à l'organisation courante.");
            ...
            group.MapPost("/", async (CreateOrderRequest request, IOrderRepository orders, IClientRepository clients, IProductRepository products, ICurrentOrganizationProvider organization) =>
            {
                ...
                var client = await clients.GetAsync(request.ClientId, organizationId) ?? 
                ...
                foreach (var line in request.Lines)
                {
                    ...
                    // Si erreur, l'exception n'est levée nulle part => le middleware s'en occupera
                    var product = await products.GetAsync(line.ProductId, organizationId) ??
                ...
                var order = new Order(Guid.NewGuid(), organizationId, client.Id, DateTime.UtcNow)
                {
            ...
            .WithDescription("Vérifie que le client et chaque produit référencé existent, qu'on est aussi sur la bonne organisation, reprend le prix courant de chaque produit dans les lignes, puis calcule le Total à partir de ces lignes.");

            // POST /orders{id}/approve — passe une commande à validée
            group.MapPost("/{id:guid}/approve", async ([Description("Identifiant de la commande à valider.")] Guid id, IOrderRepository orders, ICurrentOrganizationProvider organization) =>
            {
                //Recherche de la commande
                if (await orders.GetAsync(id,organization.GetOrganizationId()) is not { } order)
            ...
            group.MapPost("/{id:guid}/invoice", async ([Description("Identifiant de la commande à facturer.")] Guid id, IOrderRepository orders, IInvoiceRepository invoices, ICurrentOrganizationProvider organization) =>
            {
                //Recherche de la commande
                if (await orders.GetAsync(id,organization.GetOrganizationId()) is not { } order)
                    return Results.NotFound();
                ...
                //Crée la facture
                var invoice = new Invoice(Guid.NewGuid(), organization.GetOrganizationId(), order.Id, DateTime.UtcNow) { Lines = invoiceLines };
            ...
            .WithDescription("Copie les lignes de la commande dans une nouvelle facture liée à l'organisation, puis fait passer la commande au statut Facturee. Renvoie 409 si la transition n'est pas autorisée.");
